...
@section('title', 'Tambah Anak Bimbingan')
@section('breadcrumb')
<a href="{{ route('dashboard') }}">Dashboard</a><span>/</span><a href="{{ route('anak_bimbingan.index') }}">Anak Bimbingan</a><span>/</span>Tambah
@endsection
    ...
    <div class="col-lg-8">
        <div class="gkkd-card fade-in">
            <div class="gkkd-card-header">
                <h3 class="gkkd-card-title"><i class="fas fa-plus-circle me-2" style="color: var(--accent);"></i>Tambah Anak Bimbingan</h3>
            </div>
            <div class="gkkd-card-body">
                <form action="{{ route('anak_bimbingan.store') }}" method="POST">
                    @csrf
                    <div class="gkkd-form-group">
                        <label for="nama_anak" class="gkkd-form-label">Nama Anak</label>
                        <input type="text" name="nama_anak" id="nama_anak" class="gkkd-form-control" placeholder="Masukkan nama anak" value="{{ old('nama_anak') }}" required autofocus>
                    </div>
                    <div class="gkkd-form-group">
                        ...
                        <select name="pembimbing_id" id="pembimbing_id" class="gkkd-form-control" required>
                            ...
                            @foreach ($pembimbings as $pembimbing)
                                <option value="{{ $pembimbing->id }}" {{ old('pembimbing_id') == $pembimbing->id ? 'selected' : '' }}>{{ $pembimbing->nama_pembimbing }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
